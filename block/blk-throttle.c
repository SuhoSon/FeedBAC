...
static void throtl_reset_timer_fn(struct timer_list *t);

static inline struct credit_data *cpd_to_crd(struct blkcg_policy_data *cpd)
{
        return cpd ? container_of(cpd, struct credit_data, cpd) : NULL;
}

static inline struct credit_data *blkcg_to_crd(struct blkcg *blkcg)
{
        return cpd_to_crd(blkcg_to_cpd(blkcg, &blkcg_policy_throtl));
}
...
static struct blkcg_policy_data *tg_cpd_alloc(gfp_t gfp)
{
	struct credit_data *crd;
	crd = kzalloc(sizeof(*crd), gfp);
...
static void tg_cpd_init(struct blkcg_policy_data *cpd)
{
	struct credit_data *crd = cpd_to_crd(cpd);
	crd->weight = 0;
}

static void tg_cpd_free(struct blkcg_policy_data *cpd)
{
	kfree(cpd_to_crd(cpd));
}
...
#if 0
    /*
     * It the id is 2, it means that blkcg other than the root is created.
     * Therefore, start the reset_timer that calcaulted the GC contribution.
     */
    if (id == 2)
        mod_timer(&td->reset_timer, jiffies + HZ * 5);
#endif
}
...
static void throtl_find_weight (struct throtl_data *td)
{
    ...
    blkg_for_each_descendant_post(blkg, pos_css, td->queue->root_blkg) {
		...
		if (tg->weight > max_weight) {
			max_weight = tg->weight;
			td->tg[MAX] = tg;
		}

		if (tg->weight < min_weight) {
...
static void throtl_pd_offline(struct blkg_policy_data *pd)
{
    ...
    if (id == 2)
    del_timer_sync(&tg->td->reset_timer);
#endif
...
static inline void throtl_predict_crd(struct throtl_grp *tg)
{
    ...
    /* reset track latency */
    /*
       trace_printk("TOT/R:%u TOT/W:%u R:%u W:%u\n", td->nr_crd[R], td->nr_crd[W] ,crs->nr_crd[R], crs->nr_crd[W]);
       */
...
static bool tg_check_gc_insert(struct bio *bio)
{
    struct pblk *pblk = (struct pblk *)bio->bi_blkg->blkcg->private;
    struct pblk_gc *gc = &pblk->gc; 
    struct pblk_rl *rl = &pblk->rl;
    bool w_list = !(list_empty(&gc->w_list));
    int rb_gc_cnt = atomic_read(&rl->rb_gc_cnt);
    int rb_user_active;

    /* If there is no user I/O let GC take over space on the write buffer */
    rb_user_active = READ_ONCE(rl->rb_user_active);
    return w_list && (!(rb_gc_cnt >= rl->rb_gc_max && rb_user_active));
}
...
static void throtl_reset_timer_fn(struct timer_list *t)
{
    struct throtl_data *td = from_timer(td, t, reset_timer);
    struct cgroup_subsys_state *pos_css;
    struct blkcg_gq *blkg;
    ...
    mod_timer(&td->reset_timer, jiffies + HZ);
}
...
static int tg_print_weight(struct seq_file *sf, void *v)
{
        ...
        struct credit_data *crd = blkcg_to_crd(blkcg);
        unsigned int val = 0;
...
static int __tg_set_weight(struct cgroup_subsys_state *css, u64 val) {
...
{
       return __tg_set_weight(css, val);
}
...
static struct blkcg_policy blkcg_policy_throtl = {
	...
	.cpd_alloc_fn		= tg_cpd_alloc,
	.cpd_init_fn		= tg_cpd_init,
	.cpd_free_fn		= tg_cpd_free,
...
again:
    while (true) {
        ...
        if (tg->weight) {
            if(!tg_with_in_credit_limit(bio, tg))
                break;
        } else {
...
#else
void throtl_bio_crd_check(struct bio *bio)
{
#if 0
    ...
    u64 finish_time_ns;
    int rw = bio_data_dir(bio);
    unsigned int CPB = bio->bi_issue.CPB[rw];
    unsigned long finish_time, start_time;
...
int blk_throtl_init(struct request_queue *q)
{
    ...
    timer_setup(&td->reset_timer, throtl_reset_timer_fn, 0);
